...
#  Description: A program for Blackjack
...
class Player (object):
  ...
  def __str__ (self):
    ...
    return hands+'- '+ str(self.getPoints()) +' points'
...
def main ():
  ...
  game.play()
  print()
  if game.dealerPoints>21:
      for i in range(numPlayers):
        if game.playerPoints[i]<=21:
          print('Player '+str(i+1)+' wins')
        else:
          print('Player '+str(i+1)+' loses')
  else:
    for i in range(numPlayers):
      if game.dealerPoints > game.playerPoints[i]:
        print ('Player '+str(i+1)+' loses')
      elif (game.dealerPoints < game.playerPoints[i] and game.playerPoints[i] <= 21):
          print ('Player '+str(i+1)+' wins')
      elif game.dealerPoints == game.playerPoints[i]:
          if game.Players[i].hasBlackjack() and not game.dealer.hasBlackjack():
              print ('Player '+str(i+1)+' wins')
          elif not game.Players[i].hasBlackjack() and game.dealer.hasBlackjack():
              print ('Player '+str(i+1)+' loses')
          else:
              print ('Player '+str(i+1)+' ties')
      elif game.playerPoints[i]>21:
          print('Player '+str(i+1)+' loses')
main()
